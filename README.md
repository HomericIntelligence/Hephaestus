...
| Command | Description |
|---|---|
| `hephaestus-automation-loop` | Multi-repo queue-based automation pipeline using Claude Code or Codex (repo → planning → plan_review → implementation → pr_review → strict_review → merge_wait → finished; restarted implementation-GO inputs re-enter `strict_review` for the live head) |
