...
# The former auto-merge-policy exemption was removed along with that job
# (#2330): it was advisory-only and never a real gating dependency.
EXEMPT_JOBS = frozenset({GATE_JOB})
...
class TestRequiredChecksGate:
    ...
    def test_gate_does_not_need_exempt_jobs(self, jobs: dict[str, Any]) -> None:
        """The gate must not depend on itself."""
