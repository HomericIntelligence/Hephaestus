...
class TestCommandRegistry:
    ...
    def test_rejects_duplicate_names_and_aliases(self, name: str, aliases: list[str]) -> None:
        """Reject collisions without partially registering the new command."""
        registry = CommandRegistry()
...
class TestFormatOutput:
    ...
    def test_unknown_format_is_rejected(self, format_type: str) -> None:
        """Unknown, empty, and case-mismatched formats fail explicitly."""
        with pytest.raises(ValueError, match="Unsupported output format"):
            format_output({"name": "hephaestus"}, format_type=format_type)

    def test_supported_formats_remain_case_sensitive_and_compatible(self) -> None:
        """Supported format rendering remains unchanged."""
...
class TestEmitJsonStatus:
    ...
    def test_status_extra_cannot_replace_reserved_field(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
        """Status metadata cannot replace the framework-owned status field."""
        with pytest.raises(ValueError, match="status"):
            emit_json_status(0, **{"status": "error"})

        assert capsys.readouterr().out == ""

    def test_exit_code_cannot_be_supplied_as_extra(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
        """The positional exit code cannot be supplied a second time."""
        with pytest.raises(TypeError):
            emit_json_status(0, **dict[str, Any]({"exit_code": 99}))
