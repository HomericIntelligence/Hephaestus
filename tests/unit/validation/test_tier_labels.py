"""Tests for hephaestus/validation/tier_labels.py."""
...
class TestCollectMismatches:
    ...
    def test_custom_canonical_tiers(self, tmp_path: Path) -> None:
        """Custom canonical_tiers mapping is respected."""
        custom_tiers = {"T0": "Alpha", "T1": "Beta"}
        f = tmp_path / "test.md"
        # T0/Beta should be a mismatch with custom tiers (expected Alpha, found Beta)
        f.write_text("T0/Beta testing\n", encoding="utf-8")
        # With default tiers: T0/Beta is not flagged (Beta not in default names)
        findings_default = _collect_mismatches(f)
        assert findings_default == []
        # With custom tiers: still not flagged because regex only matches known names
        findings_custom = _collect_mismatches(f, canonical_tiers=custom_tiers)
        assert findings_custom == []
...
class TestScanRepository:
    ...
    def test_custom_canonical_tiers_passed_through(self, tmp_path: Path) -> None:
        """Custom canonical_tiers parameter is forwarded to _collect_mismatches."""
        (tmp_path / "file.md").write_text("T3/Delegation good\n", encoding="utf-8")
        # Default tiers: T3/Delegation is correct, no findings.
        assert scan_repository(tmp_path) == []
...
class TestTierLabelFindingFormat:
    """Tests for TierLabelFinding.format()."""

    def test_format_includes_file_and_line(self) -> None:
        """Formatted finding includes file path and line number."""
        finding = TierLabelFinding(
            file="path/to/file.md",
            line=42,
            tier="T5",
            found_name="Hierarchy",
            expected_name="Hybrid",
            raw_text="T5/Hierarchy text",
        )
        formatted = finding.format()
        assert "path/to/file.md" in formatted
