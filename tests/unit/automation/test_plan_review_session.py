...
def test_cycle_recovery_preserves_session_and_complete_transcript(tmp_path: Path) -> None:
    ...
    record = store.start_cycle(
        ...
        provider="codex",
        model="reviewer",
        reviewer_config={"model_selection_format": 1, "reasoning_effort": "medium"},
...
def test_cycle_recovery_preserves_a_provider_native_model_default(tmp_path: Path) -> None:
    """A durable cycle can keep an explicit provider-native model default."""
    store = _store(tmp_path)
    record = store.start_cycle(
        repo="org/repo",
        issue=2767,
    ...
    restarted = _store(tmp_path).recover_active(repo="org/repo", issue=2767)
...
def test_cycle_recovery_rejects_invalid_model_selection_metadata(
    reviewer_config: dict[str, object], tmp_path: Path
) -> None:
    """A malformed durable model selection must fail closed."""
    store = _store(tmp_path)
    store.start_cycle(
        repo="org/repo",
        issue=2768,
...
def test_cycle_recovery_rejects_invalid_provider_model_selection_tuple(
    ...
    with pytest.raises(PlanReviewSessionLostError, match=r"reviewer .*selection.*invalid"):
...
def test_existing_cycle_requires_reset_for_new_selection(
    ...
    with pytest.raises(PlanReviewSessionLostError, match="selection"):
        store.start_cycle(
            ...
            provider=provider,
            model=model,
            reviewer_config={"model_selection_format": 1, "reasoning_effort": effort},
            cwd=tmp_path,
            plan_revision=2,
