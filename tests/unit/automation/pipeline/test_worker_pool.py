...
class TestWorkerPoolSubmitComplete:
    ...
    @pytest.mark.skipif(sys.platform != "darwin", reason="macOS sandbox boundary")
    def test_host_profile_allows_unix_socket_in_scratch_only(self, tmp_path: Path) -> None:
        """Permit a scratch Unix socket and deny other network operations."""
        source = tmp_path / "source"
        scratch = tmp_path / "scratch"
        metadata = tmp_path / "metadata.git"
        pi_smoke_logs = source / "pi-smoke-logs"
        for directory in (source, scratch, metadata, pi_smoke_logs):
