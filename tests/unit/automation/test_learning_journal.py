...
import hephaestus.automation.learning_journal as learning_journal


def test_learning_journal_commits_only_one_claim(tmp_path: Path) -> None:
    """A durable pending intent can be claimed exactly once."""
    store = learning_journal.LearningJournalStore(lambda: tmp_path)
...
def test_learning_journal_does_not_overwrite_corrupt_record(tmp_path: Path) -> None:
    ...
    with pytest.raises(learning_journal.LearningJournalError, match="invalid JSON"):
        store.ensure_pending("key", kind="approved_plan")
...
def test_learning_journal_rejects_invalid_cleanup_state(
    ...
    with pytest.raises(learning_journal.LearningJournalError, match=message):
        store.load("key")
...
def test_learning_journal_exposes_live_claim_until_finish(tmp_path: Path) -> None:
    ...
    owner = learning_journal.LearningJournalStore(lambda: tmp_path)
    observer = learning_journal.LearningJournalStore(lambda: tmp_path)
...
def test_shared_claim_registry_survives_store_reconstruction(tmp_path: Path) -> None:
    ...
    registry = learning_journal.LearningClaimRegistry()
    owner = learning_journal.LearningJournalStore(lambda: tmp_path, claim_registry=registry)
    replacement = learning_journal.LearningJournalStore(lambda: tmp_path, claim_registry=registry)
    owner.ensure_pending("key", kind="approved_plan")
    assert owner.claim("key")
...
def test_learning_journal_validates_required_identity(tmp_path: Path) -> None:
    ...
    with pytest.raises(learning_journal.LearningJournalError, match="missing fields"):
        store.load("key")
