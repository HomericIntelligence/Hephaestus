...
@pytest.mark.parametrize("tool", PINNED_TOOLS)
def test_pinned_tool_sha256_constants_are_real_hashes(
    install_script: str, tool: str
) -> None:
...
def test_no_unverified_curl_bash_for_pinned_tools(
    install_script: str, fragment: str
) -> None:
    bad = rf"curl[^|\n]*{re.escape(fragment)}[^|\n]*\|\s*(?:bash|sh)"
