...
VALIDATOR_KINDS = {
    ...
    "github-production-rules-contracts": "repository-contract",
...
REPOSITORY_VALIDATOR_COMMANDS = {
    ...
    "workflow-contracts": (
        ...
        (
            ...
            "tests/test_postgres_slurm.py::test_full_job_resolves_owned_uv",
        ),
    ),
...
def run_repository_validator(
    ...
    *,
    ...
    """Run fixed repository commands after optional scope validation."""
    try:
    ...
    scoped_command: list[str] | None = None
    if affected_scope is not None:
        from k2_ci_policy import validate_scope
        ...
        allocation = scope["jobs"][validator_id]
        if scope["route"] == "affected" and allocation["decision"] == "run":
            scoped_command = [
                "uv",
                "run",
                "--locked",
                ...
                scope["event"]["run_id"],
                "--run-attempt",
                scope["event"]["run_attempt"],
                "--base-sha",
                scope["base"],
                "--head-sha",
                scope["head"],
            ]
    elif any(value is not None for value in (event, base, head)):
        raise ValueError("Scope context requires an explicit scope record.")
    scoped = scope is not None and scope["route"] == "affected"
    selected_commands: list[list[str]] = []
    for command in commands:
        if scoped and command[:6] == ("uv", "run", "--locked", "--extra", "dev", "pytest"):
            if scoped_command is not None:
                selected_commands.append(scoped_command)
        else:
            selected_commands.append(list(command))
    with tempfile.TemporaryDirectory(prefix="comet-actionlint-") as temporary:
        ...
        for command in selected_commands:
            actual = [str(actionlint), *command[1:]] if command[0] == "actionlint" else command
            runner(actual, cwd=repository_root, check=True)
    result: dict[str, Any] = {"commands": str(len(selected_commands)), "validator": validator_id}
    if scope is not None:
        result.update(scope=scope, decision=allocation["decision"])
    return result
...
def run_validator(
    ...
    *,
    ...
    if kind == "repository-contract":
        return run_repository_validator(
            validator_id,
            repository_root,
    ...
    if affected_scope is not None or any(value is not None for value in (event, base, head)):
        raise ValueError("Affected scope applies only to repository contracts.")
