...
| `pipeline/stages/strict_review.py:StrictReviewStage` | `Read,Glob,Grep` | Independent per-head review runs through `AgentJob(sandbox="read-only")`; the coordinator alone publishes and validates its artifact. |
