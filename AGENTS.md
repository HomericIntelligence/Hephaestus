...
This file is the sole authoritative agent contract for Hephaestus. It is a
single-page map of the AI-agent topology and conventions used by Hephaestus and
the wider HomericIntelligence ecosystem, together with the project-specific
rules (commit policy, branch naming, version model) that previously lived in
`CLAUDE.md`. For enabled skill plugins see
[`.claude/settings.json`](.claude/settings.json), and for invocation guidance
see the [Skill catalog](#skill-catalog).
...
the agents can invoke. Highlights:
...
skill applies. Use `Skill(skill: "athena:skill-advisor", args: "<task description>")`. See the
[Skill catalog](#skill-catalog) for the full table of available skills.
