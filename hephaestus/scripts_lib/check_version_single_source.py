...
def check_pyproject_dynamic_version(repo_root: Path) -> bool:
    ...
    if not pyproject_path.exists():
        print(text("ERROR: pyproject.toml not found at %(value0)s", value0=pyproject_path))
        return False
    ...
    print(text("OK: pyproject.toml uses hatch-vcs dynamic versioning (single source)"))
    return True
