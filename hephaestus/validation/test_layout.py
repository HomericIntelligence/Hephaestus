"""Validate the repository's unit-test layout.
...
def _report_mirror_check(
    ...
    if mirrored:
        if verbose:
            ...
            print(
                text("OK: All %(count)d source subpackages have test directories.", count=src_count)
            )
        return True
    print(
        text(
    ...
    for name in sorted(missing):
        print(
            text(
                "  %(package)s/%(name)s  ->  tests/unit/%(name)s/ (missing)",
                package=src_package,
                name=name,
            ),
            file=sys.stderr,
...
def _report_loose_files_check(test_root: Path, verbose: bool) -> bool:
    ...
    for p in violations:
        print(text("  %(path)s", path=p), file=sys.stderr)
    return False
...
def _report_unsanctioned_dirs_check(
    ...
    print(
        text(
            ...
            "comment) in hephaestus/validation/test_layout.py, or remove the dir.\n"
            "Unsanctioned:"
        ),
        file=sys.stderr,
    )
    for name in sorted(unsanctioned):
        print(text("  tests/unit/%(name)s/", name=name), file=sys.stderr)
    return False
...
def _report_ghost_packages_check(
    ...
    for name in sorted(ghosts):
        print(
            text(
                "  hephaestus/%(name)s/ (no modules)  <->  tests/unit/%(name)s/ (no tests)",
                name=name,
            ),
            file=sys.stderr,
        )
...
def _report_phantom_test_dirs_check(test_root: Path, verbose: bool) -> bool:
    ...
    for name in sorted(phantoms):
        print(text("  tests/unit/%(name)s/ (no Python source)", name=name), file=sys.stderr)
    return False
...
def _report_scripts_coverage_check(
    ...
    if ok_scripts:
        if verbose:
            ...
            print(text("OK: %(count)d scripts/*.py covered by the smoke harness.", count=n_scripts))
        return True
...
def _format_scripts_coverage_error(error: str) -> str:
    """Translate one authored smoke-harness diagnostic."""
    if error == (
        "  The scripts/ smoke harness is required so every scripts/*.py is auto-tested via --help."
    ):
        return text(error)
    if match := re.fullmatch(r"  Missing: tests/unit/scripts/(?P<name>.+)", error):
        return text("  Missing: tests/unit/scripts/%(name)s", **match.groupdict())
    if error == (
...
def check_test_structure(
    ...
    if not src_root.is_dir():
        print(text("ERROR: Source root not found: %(path)s", path=src_root), file=sys.stderr)
        return False

    if not test_root.is_dir():
        print(text("ERROR: Test root not found: %(path)s", path=test_root), file=sys.stderr)
        return False
