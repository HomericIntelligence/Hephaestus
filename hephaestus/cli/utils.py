...
def emit_json_status(exit_code: int, message: str | None = None, **extra: Any) -> None:
    ...
    Raises:
        ValueError: If an extra field attempts to replace ``status`` or
            ``exit_code``.
...
def format_output(data: Any, format_type: str = "text") -> str:
    """Format data using a supported, case-sensitive output format.
    ...
    Raises:
        ValueError: If ``format_type`` is not a supported output format.
...
_POSITIVE_TIMEOUT_HELP = " Must be a positive integer; zero does not disable the timeout."


def _positive_int(value: str) -> int:
    """Parse a strictly positive integer for timeout CLI arguments."""
    try:
        parsed = int(value)
    except ValueError as exc:
...
def add_advise_timeout_arg(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        metavar="SECONDS",
        help=(
            f"Timeout for the advise sub-agent in seconds (default: 7200).{_POSITIVE_TIMEOUT_HELP}"
        ),
    )
...
def add_learn_timeout_arg(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="SECONDS",
        help=(f"Timeout for the /learn agent session (default: 7200).{_POSITIVE_TIMEOUT_HELP}"),
    )
