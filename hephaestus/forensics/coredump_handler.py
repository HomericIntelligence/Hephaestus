...
from hephaestus.cli.localization import get_localizer, text
...
def verify_crash_bundle(log_dir: Path, *, localize: bool = True) -> tuple[str, str]:
    ...
    Args:
        ...
        localize: If true, translate the human-readable detail.
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "pid",
        nargs="?",
        help=get_localizer().template("PID of the crashing process (%%p)"),
    )
    parser.add_argument(
        "exe",
        nargs="?",
        help=get_localizer().template("executable basename (%%e)"),
    )
    parser.add_argument(
        "crash_time",
        nargs="?",
        help=get_localizer().template("crash time, seconds since epoch (%%t)"),
    )
    parser.add_argument(
        "signal",
        nargs="?",
        help=get_localizer().template("signal number (%%s)"),
    )
    parser.add_argument(
        "global_pid",
        nargs="?",
        default="",
        help=get_localizer().template(
            "global (host-namespace) PID (%%P) — captured, unused in the filename"
        ),
    )
    add_json_arg(parser)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if missing:
        ...
        if args.json:
            ...
            print(
                text(
                    "hephaestus-coredump-handler: %(message)s",
                    message=text(
