...
CODEX_ATHENA_ARTIFACT_SHA256 = "9e7bbba3befe93916cc4f1d944bc667cadd41dffb0f298199b9dbb6f6fe39ed8"
...
def _athena_artifact_digest(root: Path) -> str:
    """Hash the admitted Athena source tree, excluding generated bytecode."""
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.is_symlink():
            raise AgentExecutionError(
                "Codex automation rejects symlinks in the Athena plugin cache"
            )
        if not path.is_file():
            continue
...
def _codex_automation_profile() -> Iterator[dict[str, str]]:
    ...
    source_digest = _athena_artifact_digest(athena_cache_source)
    ...
    with tempfile.TemporaryDirectory(prefix="hephaestus-codex-") as temporary:
        ...
        if _athena_artifact_digest(destination_artifact) != source_digest:
        ...
        write_secure(
            ...
            "\n".join(
                (
                    ...
                    '":minimal" = "read"',
                    f'{json.dumps(str(destination_artifact))} = "read"',
...
def _codex_base_cmd(
    *,
    ...
    if resume_id is not None:
        ...
        # An automation profile selects Codex's named permission profile from
        # its isolated config.toml.  The legacy sandbox command surface does
        # not compose with permission profiles and would re-grant the shared
        # Git common directory through --add-dir.
        if sandbox is not None and not automation_profile:
            cmd.extend(["--sandbox", sandbox])
        if not automation_profile:
...
def run_codex_session(
    ...
    *,
    ...
    cmd = _codex_base_cmd(
        ...
        sandbox=sandbox,
        approval=approval,
        automation_profile=isolate_automation_profile,
    )
    ...
    *,
    ...
    with tempfile.NamedTemporaryFile(prefix="codex-last-", suffix=".txt") as output_file:
        ...
        with profile as env:
            ...
            if isolate_automation_profile:
                profile_root = Path(env["CODEX_HOME"])
                output_path = profile_root / "last-message.txt"
                write_secure(output_path, "")
                cmd[cmd.index("--output-last-message") + 1] = str(output_path)
                ...
                try:
                    cmd = macos_isolated_command(
                        cmd,
                        read_roots=(cwd, profile_root),
                        write_roots=(cwd, profile_root),
                        profile_directory=profile_root,
                        # The Codex provider needs its transport.  The named
                        # permission profile denies network access to commands.
                        allow_network=True,
                    )
                except ProcessIsolationError as exc:
                    raise AgentExecutionError(f"Codex automation isolation failed: {exc}") from exc
            try:
