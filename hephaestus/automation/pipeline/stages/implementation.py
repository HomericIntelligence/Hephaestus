...
#: Host-owned launcher that copies the candidate through a no-follow walk and
#: executes only the anonymous snapshot whose bytes it verifies. The fixed
#: bound limits a candidate-controlled file read before any code runs.
...
def _trusted_host_executable(name: str) -> str:
    """Return one system-owned executable path or an empty fail-closed value."""
...
def _hephaestus_required_check_job_argv(item: WorkItem) -> tuple[str, ...]:
    ...
    runner_shell = HEPHAESTUS_REQUIRED_CHECK_ARGV[0]
