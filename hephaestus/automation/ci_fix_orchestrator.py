...
class CIFixOrchestrator:
    ...
    def build_ci_fix_prompt(
        ...
        return (
            ...
            "HARD SCOPE LIMITS: never rewrite, truncate, or 'clean up' README.md "
            "or other documentation unless a listed failing check is explicitly "
            "about that file — and even then, make the smallest edit that "
            "satisfies the check (e.g. update a badge count), never replace the "
            "file with a placeholder. Never delete or stub out existing project "
            "content to silence a failure you cannot fix; if you cannot fix a "
            "failure, say so and stop instead of committing unrelated changes. "
            "After fixing:\n"
