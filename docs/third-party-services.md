...
| Pi private provider (optional) | Operator-local direct adapter smoke path (`docs/pi-private-provider.md`); normal automation remains fail-closed pending #2516/#2518 | Low — optional adapter, not yet an automation provider | Operator-local configuration only; documentation uses placeholders (ADR-0005 / `docs/pi-private-provider.md`) | Provider-local availability | Operator-managed |
| npm registry | Installs the exact top-level Pi coding-agent CLI in every unit-test matrix lane and the required unit-test job (`.github/actions/setup-pi-cli`) | High for unit CI — an unavailable or changed artifact blocks required unit checks | Maintain the exact CLI version and `--ignore-scripts`; package-artifact admission, updates, and integrity policy begin in #2516, and no repository-owned npm lock/integrity assertion exists yet | Registry availability | <https://status.npmjs.org/> — best-effort |
