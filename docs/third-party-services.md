...
| Anthropic (Claude CLI / API) | Primary agent provider for the automation pipeline (the loop shells out to the `claude` CLI) | High — automation halts; manual development is unaffected | Quota monitoring, operator-supplied model fallback, circuit breaker and retry (`hephaestus/resilience/`), the [claude-quota-exhausted](runbooks/claude-quota-exhausted.md) runbook | API availability, model behavior and quotas | <https://status.anthropic.com/> — usage-tier limits, no SLA |
...
| Pi private provider (optional) | Managed Pi automation selected with `--agent pi`; normal automation is N/A unless a host registers a reviewed OS-isolation adapter | Low — optional adapter, not yet an automation provider | Operator-local configuration only; documentation uses placeholders and must provide the host isolation broker (ADR-0005 / `docs/pi-private-provider.md`) | Provider-local availability | Operator-managed |
| npm registry | Supplies the catalog-pinned Pi CLI, `pi-subagents`, and `pi-web-access` artifacts for the managed Pi bootstrap path | High for Pi bootstrap and live smoke | Maintain reviewed exact pins, disable lifecycle scripts, verify installed identity/version and capability provenance, and run gated package smoke evidence | Registry availability and artifact serving | <https://status.npmjs.org/> — best-effort |
