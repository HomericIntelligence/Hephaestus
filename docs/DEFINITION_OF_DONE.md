...
| 8 | `uv run mypy` returns `Success: no issues found in N source files` | CI job `lint` |
...
| 13 | Shell tests pass: `uv run test-shell` | CI job `shell-tests` |
| 14 | Schema validation passes (CLI inventory, YAML/Markdown structure) | CI job `schema-validation` |
| 15 | Dep sync check passes (pyproject.toml then pyproject.toml then uv.lock) | CI job `deps/version-sync` |
