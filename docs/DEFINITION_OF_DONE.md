...
| 4 | Auto-merge is armed only by `merge_wait`, after `strict_review` has run `$athena:pr-review` for the current head and applied loop-owned `state:implementation-go`. | Queue gate; advisory `auto-merge-policy` and human review |
