...
| 3 | Every commit on the branch is cryptographically signed and DCO-signed (`git commit -S -s`) | `homeric-main-baseline` ruleset (`required_signatures`) + CI `pr-policy` signature and DCO checks |
...
> reports required workflow results; it does not authorize automatic merge. The
> automation loop's in-process review result and its
