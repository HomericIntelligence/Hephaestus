...
| 4 | Auto-merge is armed only by `merge_wait`, after `pr_review` has run `$athena:pr-review` in its default profile (or the inline fallback) and applied loop-owned `state:implementation-go` on GO. | Queue gate and human review |
...
> The automation loop's in-process review result and its
> `state:implementation-go` label, rather than any reporting check, are the
