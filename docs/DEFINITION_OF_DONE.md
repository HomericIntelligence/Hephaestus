...
| 4 | `pr_review` writes loop-owned `state:implementation-go` only after `$athena:pr-review` (or the inline fallback) and a current-process reviewed-head proof. `merge_wait` revalidates that proof against a confirmed-unarmed `main` PR and makes one ordinary squash merge conditional on that SHA; no queue stage mutates auto-merge. | Queue gate; advisory `auto-merge-policy` and human review |
