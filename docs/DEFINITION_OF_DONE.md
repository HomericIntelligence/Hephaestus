...
| 4 | Auto-merge is armed only by `merge_wait`, after `strict_review` has run `$athena:pr-review` for the current head, applied `state:implementation-go`, and directly handed off that head. | Queue gate; advisory `auto-merge-policy` and human review |
