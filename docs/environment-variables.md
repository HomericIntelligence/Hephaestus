...
| `GIT_ALTERNATE_OBJECT_DIRECTORIES` | child-process | config.child_environments | Check-only candidate Git alternate object directories | public | non-empty | input | `hephaestus/config/child_environments.py:build_check_only_git_env:read` |
| `GIT_INDEX_FILE` | child-process | config.child_environments | Bound implementation Git index | public | path | input | `hephaestus/config/child_environments.py:build_check_only_git_env:read` |
| `GIT_OBJECT_DIRECTORY` | child-process | config.child_environments | Check-only candidate Git object directory | public | path | input | `hephaestus/config/child_environments.py:build_check_only_git_env:read` |
