...
| `state:implementation-go` | Confirmed exclusive PR label applied by `pr_review` only when structural audit facts and a complete GitHub thread read show no blocking or human thread on the exact reviewed open/unarmed head. Review prose, grades, and model decision text are informational. Merge-wait also needs the current process's reviewed-head proof and currently stands by pending #2419 (ADR-0014). |
| `state:implementation-no-go` | Structural review found unresolved blocking implementation facts on the current reviewed head; implementation needs re-work. |
| `state:skip` | Work item taken out of the loop entirely — operator-applied, auto-applied when the review loop exhausts its budget without a confirmed clear structural review, or applied to epics before exclusion. Independent of all other state labels. See [Reviving a state:skip-labeled issue](state-skip-revival.md) to safely clear it. |
