...
| `state:implementation-go` | Confirmed exclusive PR label applied by `pr_review` only when structural facts, complete live GitHub thread facts, and the exact reviewed open/unarmed head pass both pre- and post-write checks. Review prose, grades, CI, and model decision text are informational. Merge-wait also needs the current process's reviewed-head proof and currently stands by pending #2419 (ADR-0014). |
...
| `state:skip` | Work item taken out of the loop entirely — applied by an operator, by review-budget exhaustion without confirmed clear structural facts, or to epics before exclusion. Independent of all other state labels. See [Reviving a state:skip-labeled issue](state-skip-revival.md) to safely clear it. |
