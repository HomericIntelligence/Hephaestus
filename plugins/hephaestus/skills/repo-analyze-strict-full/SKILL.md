---
name: repo-analyze-strict-full
description: Ruthlessly thorough repository audit with strict grading AND full file coverage - dispatches one Myrmidon swarm agent per section so every file is read, then grades from F up with concrete evidence required.
