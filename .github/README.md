...
`homeric-main-baseline` ruleset. The automation loop runs `$athena:pr-review`
and owns the `state:implementation-go` label. `merge_wait` may then make a bounded sequence
